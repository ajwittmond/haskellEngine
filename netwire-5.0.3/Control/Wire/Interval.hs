...
--
-- Impedance value is the current remaining time

after :: (HasTime t s, Monoid t) => t -> Wire s t m a a
after t' =
    mkPure $ \ds x ->
        let t = t' - dtime ds in
        if t <= 0
...
asSoonAs :: (Monoid e) => Wire s e m (Event t a) a
asSoonAs = hold
...
between :: (Monoid e) => Wire s e m (a, Event t b, Event t' c) a
between =
    mkPureN $ \(x, onEv, _) ->
...
--
-- Impedance value is the current time subtracted from when impedance began

for :: (HasTime t s,Monoid t) => t -> Wire s t m a a
for t' =
    mkPure $ \ds x ->
        let t = t' - dtime ds in
        if t <= 0
...
hold :: (Monoid e) => Wire s e m (Event t a) a
hold =
    mkPureN $
        event (Left mempty, hold)
              ((Right &&& holdWith) . snd . last)
...
--
-- Impedance value is the current time subtracted from
-- the time that the given amount expired

holdFor :: (HasTime t s, Monoid t) => t -> Wire s t m (Event t a) a
...
until :: (Monoid e) => Wire s e m (a, Event t b) a
until =
    mkPureN . uncurry $ \x ->
